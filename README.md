...
- **EQ Field** — ✅ *done (2026-07)*. Field's login trusts the Shell-verified `eq_role` from the JWT (Phase D) for Shell-handoff sessions and keys its access on the canonical `EqRole` enum. The two **coarse** field gates (`field.view`, `field.dispatch`) live here and are consumed by Shell. Field's ~78 **fine-grained** in-app permissions (`roster.*`, `ts.*`, `ph.*`, `leave.*`, `people.*`, `sites.*`, `app.*`, `reports.*`, `admin.*`) remain **Field-owned** (a hand-maintained matrix in `scripts/permission-matrix.js`, keyed to these same roles); a Field-side startup guard warns if its role keys ever drift from `ROLE_KEYS` here. `subcontractor` is intentionally **excluded** from Field — it is a roster `employment_type`, never a Field login role. **Caveat (verified 2026-08-06, re-verified 2026-08-07):** Field carries no npm dependency on this package — `ROLE_KEYS` is hand-copied (`EQ_ROLE_KEYS` in `verify-pin.js`, not yet vendored; the client-side startup guard's own comparison target was the same kind of hand-copy until eq-field PR #658, which pointed it at a real vendored file instead). Only 4 of the ~78 fine-grained keys (`field.manage_roster/licences/labour_hire/people`) are reachable through Shell's `extra_perms` — the other ~74 are invisible to Shell's Access Control page. A legacy `staff`/`supervisor` squash exists in `verify-pin.js` for tenants with no Shell handoff, but on closer inspection (2026-08-07) it's **not an active fallback** — it's been server-gated unreachable for every real tenant since PR #575 (2026-07-30), kept intentionally only for a hypothetical future non-Core-only tenant. Separately (not this package's concern, but found in the same investigation): Shell's *own* role-matrix permission overrides don't currently reach Field's JWT at all — a pipeline bug in eq-shell, not a role-model gap.
...
- **Cards** — ✅ *emit ready (2.5.0)*, ✅ *installable as a real Dart package (2.5.5)*. `lib/eq_roles.dart` is generated (Dart 2.17+ enhanced enums, zero external deps), mirrors `roles.ts`/`roles.js` exactly, and is guarded against drift by `roles.dist.test.ts`. This repo now ships a `pubspec.yaml` so Cards can depend on it via `git:` the same way it already depends on `eq_design_tokens` — see eq-cards for wiring status.
